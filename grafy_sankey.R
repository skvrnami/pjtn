...
vztahy <- data %>% 
    filter(grepl("Charakteristika.Vztahy", path)) %>% 
    mutate(path_original = path) %>% 
    tidyr::separate(., col = path, sep = "\\.", 
                    into = c("lvl1", "lvl2", "lvl3", "lvl4")) %>% 
    arrange(path_original) %>% 
...
vztahy_levels <- c(
    ...
    "rodič opatrovník (33)", 
    "vrstevníci (45)", 
    "lékařská a terapeutická péče (3)", 
    "církevní představitel (5)", 
    "volnočasové aktivity (11)", 
    "práce (13)", 
    "vzdělávání (13)", 
    "veřejně známá osobnost a autorita (16)", 
    "velký věkový rozdíl (100)", 
    "v partnerství figurují potomci (6)", 
    "manželství (6)", 
    "bývalí partneři (17)", 
    "práce (11)", 
    "Neznámá osoba (51)", 
    ...
    "Známí (155)", 
    "Rodinné (156)", 
    "Mocensky asymetrické vztahy (162)", 
    "Partnerské (209)", 
    "Věk (547)", 
    "Vztahy (884)"
    )

vztahy_long <- make_long(vztahy, lvl2, lvl3, lvl4) %>% 
    filter(!is.na(node)) %>% 
    mutate(node = factor(node, levels = vztahy_levels),
           next_node = factor(next_node, levels = vztahy_levels))

ggplot(vztahy_long, aes(x = x, 
                      next_x = next_x, 
                      node = node, 
                      # node = factor(node, levels = node_levels2),
                      label = node, 
    ...
    guides(fill = "none") +
    theme_sankey(base_family = "Poppins") + 
    labs(title = "Proč jsme to nenahlásily*i: vztahy", 
         subtitle = "Témata zmiňované ve výpovědích a jejich četnost") + 
    theme(axis.title.x = element_blank(), 
          axis.text.x = element_blank())

ggsave("figs/sankey_vztahy.png", width = 10, height = 7)
